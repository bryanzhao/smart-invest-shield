...
function ActorsView() {
  ...
  return <section className="view-panel"><div className="view-intro"><div><span className="section-kicker">竞争格局 / ACTOR INTELLIGENCE</span><h2>竞品与关键人物</h2><p>将组织、人物、项目和公开动作放在同一张可验证的关系网络中。</p></div><button className="secondary-button"><Network size={15} /> 查看关系图</button></div><div className="actor-grid">{actors.map((actor) => <article className="actor-card" key={actor.name}><div className={`actor-avatar actor-avatar-${actor.tone}`}>{actor.type === "company" ? <Building2 size={20} /> : <UserRound size={20} />}</div><div className="actor-card-top"><span className="actor-type">{actor.type === "company" ? "公司档案" : "人物档案"}</span><span className="evidence evidence-a">A</span></div><h3>{actor.name}</h3><p className="actor-role">{actor.role}</p><div className="actor-signal"><Activity size={14} /><span>{actor.signal}</span></div><div className="actor-footer"><span>最后验证 · 2 天前</span><ArrowUpRight size={15} /></div></article>)}</div></section>;
}
